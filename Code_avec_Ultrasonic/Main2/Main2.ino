...
const int reverseSpeed = 1620;    // reverse //1570 1550 //  new one for min : 1620
                                  //  // max new 1960
...
//Thresholds
const int frontThreshold = 50;    //Thresholds for the different 'types' of
                                  // sonar sensors front,sides, and Back.In
                                  // centimeters.
const int sideThreshold = 15;
  ...
  if (distance < frontThreshold && distance != 0) {
    ...
    if (distance < frontThreshold && distance != 0) {
      ...
      if (distance < frontThreshold && distance != 0) {
        ...
        Serial.println(distance);
        delay(8);
        if (distance > frontThreshold || distance == 0) { // || distance == 0
